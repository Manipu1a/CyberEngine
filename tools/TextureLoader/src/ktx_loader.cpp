...
TEXTURE_FORMAT get_ktx_format(std::uint32_t GLInternalFormat)
{
...
{
    static constexpr uint8_t KTX10FileIdentifier[12] = {0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A};
    if(dataSize >= 12 && memcmp(data, KTX10FileIdentifier, sizeof(KTX10FileIdentifier)) == 0)
    {
        data += sizeof(KTX10FileIdentifier);
        const KTX10Header& ktxHeader = *reinterpret_cast<const KTX10Header*>(data);
        data += sizeof(KTX10Header);
        // skip the key-value data
        data += ktxHeader.BytesOfKeyValueData;

        m_textureCreateDesc.m_format = get_ktx_format(ktxHeader.GLInternalFormat);
        if(m_textureCreateDesc.m_format == TEX_FORMAT_UNKNOWN)
        {
            cyber_error("Unsupported KTX format: {0}", ktxHeader.GLInternalFormat);
        }

        m_textureCreateDesc.m_width = ktxHeader.Width;
        if( m_textureCreateDesc.m_width == 0)
        {
            cyber_error("KTX texture width is zero, which is invalid.");
        }
        m_textureCreateDesc.m_height = std::max(ktxHeader.Height, 1u);

        const auto src_mipmap_levels = std::max(ktxHeader.NumberOfMipmapLevels, 1u);
        m_textureCreateDesc.m_mipLevels = src_mipmap_levels;
        if(texLoadInfo.mipLevels > 0)
        {
            m_textureCreateDesc.m_mipLevels = std::min(texLoadInfo.mipLevels, src_mipmap_levels);
        }

        const auto num_faces = std::max(ktxHeader.NumberOfFaces, 1u);
        const auto array_size = std::max(ktxHeader.NumberOfArrayElements, 1u)* num_faces;
        if(num_faces == 1)
        {
            if(ktxHeader.Depth >= 1)
            {
                m_textureCreateDesc.m_dimension = TEX_DIMENSION_3D;
                m_textureCreateDesc.m_depth = ktxHeader.Depth;
            }
            else
            {
                m_textureCreateDesc.m_dimension = array_size > 1 ? TEX_DIMENSION_2D_ARRAY : TEX_DIMENSION_2D;
                m_textureCreateDesc.m_arraySize = array_size;
            }
        }
        else if(num_faces == 6)
        {
            m_textureCreateDesc.m_dimension = array_size > 6 ? TEX_DIMENSION_CUBE_ARRAY : TEX_DIMENSION_CUBE;
            m_textureCreateDesc.m_arraySize = 6;
        }
        else 
        {
            cyber_error("Unsupported number of faces in KTX texture: {0}", num_faces);
        }

        m_textureSubResData.resize(size_t(m_textureCreateDesc.m_mipLevels) * array_size);

        for(uint32_t mip = 0; mip < src_mipmap_levels; ++mip)
        {
            data += sizeof(uint32_t);
            auto mip_info = compute_mip_level_properties(m_textureCreateDesc, mip);

            for(uint32_t layer = 0; layer < array_size; ++layer)
            {
                if(mip < m_textureCreateDesc.m_mipLevels)
                {
                    m_textureSubResData[mip + size_t(layer) * m_textureCreateDesc.m_mipLevels] = RenderObject::TextureSubResData{data, mip_info.row_size, mip_info.depth_slice_size};
                }
                data += align_up(mip_info.mip_size, 4U); // KTX stores mip data aligned to 4 bytes
            }
        }
    }
}
