...
namespace Cyber
{
    namespace RenderObject
    {
        ...
        struct CYBER_GRAPHICS_API IRenderDevice
        {
            ...
            virtual void wait_fences(RHIFence** fences, uint32_t fenceCount) = 0;
            virtual void free_fence(RHIFence* fence) = 0;
            virtual ERHIFenceStatus query_fence_status(RHIFence* pFence) = 0;
            virtual RHISwapChain* create_swap_chain(const RHISwapChainCreateDesc& swapchainDesc) = 0;
            ...
            virtual void present_queue(RHIQueue* queue, const RHIQueuePresentDesc& presentDesc) = 0;
            virtual void wait_queue_idle(RHIQueue* queue) = 0;
            virtual void free_queue(RHIQueue* queue) = 0;
            // Command APIs
            virtual RHICommandPool* create_command_pool(RHIQueue* pQueue, const CommandPoolCreateDesc& commandPoolDesc) = 0;
            virtual void reset_command_pool(RHICommandPool* pPool) = 0;
            virtual void free_command_pool(RHICommandPool* pPool) = 0;
            virtual RHICommandBuffer* create_command_buffer(RHICommandPool* pPool, const CommandBufferCreateDesc& commandBufferDesc) = 0;
            virtual void free_command_buffer(RHICommandBuffer* pCommandBuffer) = 0;
            /// RootSignature
            ...
            /// CMDS
            virtual void cmd_begin(RHICommandBuffer* cmd) = 0;
